...
def parse_args(argv):
    """Parse query, --count N, --months N, --top-views N, and --excel <filename> from argv."""
    args = argv[1:]
    count = 20
    months = 6
    top_views = None
    ...
    while i < len(args):
        if args[i] == "--count" and i + 1 < len(args):
            ...
            try:
                ...
                sys.exit(1)
            i += 2
        elif args[i] == "--top-views" and i + 1 < len(args):
            try:
                top_views = int(args[i + 1])
            except ValueError:
                print(f"Error: --top-views requires an integer, got '{args[i + 1]}'", file=sys.stderr)
                sys.exit(1)
            i += 2
        elif args[i] == "--excel" and i + 1 < len(args):
            excel_file = args[i + 1]
            i += 2
    ...
    if not query:
        print("Usage: search.py <query> [--count N] [--months N] [--no-date-filter] [--top-views N] [--excel <filename>]", file=sys.stderr)
        print("Example: search.py claude code tutorial --count 5 --months 3 --top-views 5 --excel results.xlsx", file=sys.stderr)
        sys.exit(1)
    return query, count, months, top_views, excel_file
...
def export_to_excel(videos, excel_file):
    """Export video data to Excel file."""
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "YouTube Search Results"

    # Headers
    headers = ["#", "Title", "Channel", "Views", "Subscribers", "Views/Sub", "Duration", "Upload Date", "URL"]
    ws.append(headers)
    ...
    for i, info in enumerate(videos, 1):
        ...
        row = [i, title, channel, views, subs_str, ratio_str, duration, date, url]
        ws.append(row)
...
def main():
    query, count, months, top_views, excel_file = parse_args(sys.argv)

    if not shutil.which("yt-dlp"):
        print("Error: yt-dlp not found on PATH. Install with: pip install yt-dlp", file=sys.stderr)
        sys.exit(1)

    # Check Excel availability if requested
    if excel_file and not EXCEL_AVAILABLE:
        print("Error: Excel export requested but openpyxl is not installed.", file=sys.stderr)
    ...
    fetch_count = count
    if top_views:
        fetch_count = max(fetch_count, top_views * 3)
    if months > 0:
        fetch_count = max(fetch_count, count * 3)
    fetch_count = max(fetch_count, count)
    ...
    for cookies_file in possible_cookies:
    ...
    # Sort by view count if --top-views is specified
    if top_views:
        videos = sorted(videos, key=lambda v: v.get("view_count", 0), reverse=True)
        videos = videos[:top_views]
        print(f"(Selected top {top_views} videos by view count)\n", file=sys.stderr)
    else:
        # Otherwise just limit to requested count
        videos = videos[:count]

    # Export to Excel if requested
    if excel_file:
        export_to_excel(videos, excel_file)
    ...
    for i, info in enumerate(videos, 1):
        ...
        subs_str = format_subscribers(subs)
        meta = f"{channel} ({subs_str} subs)  ·  {views_str} views  ·  {ratio_str} views/sub  ·  {duration}  ·  {date}"
